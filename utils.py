...
def neighbors(fringe, A, outgoing=True):
    # For all n in fringe, return a list of neighbor indices
    # O(n*fringe) ~~ O(n^2)?
    ...
    return res
def process_AA_subgraphs(A, link_index, n=None, prob=1,reverse=False):
    # Processes all subgraphs indexed by ind in edge_index
    multiplier = 1 / np.log(A.sum(axis=0))
    multiplier[np.isinf(multiplier)] = 0
    A_ = A.multiply(multiplier).tocsr()
    scores = []
    for src,dst in tqdm(link_index):
        AA_scores = A[src].multiply(A_[dst]).toarray()
        sorted_array = np.argsort(AA_scores).flatten()
        if reverse:
            if n is None:
                final = sorted_array[:int(len(sorted_array)*prob)]
            else:
                final = sorted_array[:int(n)]
...
def get_n_neighbors(src,dst, A, n=None, prob=1):
    # calculate AA score for all nodes
    # return n largest scored nodes
    pruned = []
    multiplier = 1 / np.log(A.sum(axis=0))
    multiplier[np.isinf(multiplier)] = 0
    A_ = A.multiply(multiplier).tocsr()
...
def get_subgraph(src,dst,A,max_nodes=None,node_features=None,ratio=1.0,y=1):
    nodes = [src,dst]
    dists = [0,0]
    ...
    subgraph[0,1]=0
    subgraph[1,0]=0
    transformed = rts(subgraph)    
    if node_features is not None:
        node_features = node_features[nodes]
                
    return nodes, transformed, dists, node_features, y
...
def get_k_hop_subgraph(src,dst,A,k,max_nodes=None,node_features=None,ratio=1.0,y=1):
    ...
    # et = time.time()
    # print("Overhead: ", et-st)
    # st = time.time()
    subgraph = A[nodes, :][:, visited]
    ...
    # et = time.time()
    # print("Transformation: ", et-st)
    # exit()
    return nodes, transformed, dists, node_features, y  
## Row to square function using CSC Matrix
def rts(r:ssp.csr_matrix):
...
def k_hop_subgraph(src, dst, num_hops, A, sample_ratio=1.0,
                   ...
                   y=1, directed=False, A_csc=None, AA_subgraph=None):
    # O(n?)
    # Extract the k-hop enclosing subgraph around link (src, dst) from A.
    st = time.time()
    get_subgraph(src,dst,A)
    et = time.time()
    print("My Time: ", et-st)
    st = time.time()
    nodes = [src, dst]
    ...
    for dist in range(1, num_hops+1):
        if dist == 1 and AA_subgraph is not None:
            # first hop
            fringe = set(AA_subgraph)
            pass
        elif not directed:
            fringe = neighbors(fringe, A)
        else:
        ...
        if max_nodes_per_hop is not None:
            if max_nodes_per_hop < len(fringe):
                ...
                pass
                # fringe = get_n_neighbors(src,dst,fringe,A,max_nodes_per_hop)
        if len(fringe) == 0:
            break
        nodes = nodes + list(fringe)
...
def extract_enclosing_subgraphs(link_index, A, x, y, num_hops, node_label='drnl',
    ...
    data_list = []
    # AA_subgraph = process_AA_subgraphs(A,link_index,max_nodes_per_hop,ratio_per_hop)
    idx = 0
    for src, dst in tqdm(link_index.t().tolist()):
        tmp = k_hop_subgraph(src, dst, num_hops, A, ratio_per_hop,
                             max_nodes_per_hop, node_features=x, y=y,
                             directed=directed, A_csc=A_csc, AA_subgraph=None)
