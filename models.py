...
class DGCNN(torch.nn.Module):
    def __init__(self, hidden_channels, num_layers, max_z=1000, k=0.6, train_dataset=None,
        ...
        print("DGCNN")
        print(f"hidden_channels: {hidden_channels}, numLayers = {num_layers}, use_feature: {use_feature}, node_embedding: {node_embedding}")
        ...
        # for seal no features are used
        if self.use_feature:
            initial_channels += train_dataset.num_features

        # for seal no node embeddings are used
        if self.node_embedding is not None:
            initial_channels += node_embedding.embedding_dim
        print("Initial Channels: ", initial_channels, "| K: ", self.k, " | Z_embedding: ", self.z_embedding)
